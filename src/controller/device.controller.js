...
const Sensores_Integradora = require('../model/test.model.js'); 
const AlertSchema = require('../model/test.model.js'); 
...
const getDevicesYo = async (req, res) => {
  try {
      const dispositivo = await Sensores_Integradora.find({});
...
const getAlerta = async (req, res) => {
    ...
    try {
        const sensorAlerts = alerta.sensorType.alerts;
...
const getAlertas = async (req, res) => {
    try {
        const alerta = await AlertSchema.find({});
        res.json(alerta);
    } catch (error) {
        res.status(500).send(error.message);
    }
